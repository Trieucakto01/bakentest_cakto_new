...
#include "../driver/drv_deye_solarman.h"
...
int http_fn_index(http_request_t* request) {
	...
	if (!http_getArg(request->url, "state", tmpA, sizeof(tmpA)) && DRV_IsRunning("DeyeSolarman")) {
		if (http_getArg(request->url, "restart", tmpA, sizeof(tmpA))) {
			RESET_ScheduleModuleReset(3);
		}
		return DeyeSolarman_HTTPIndexPage(request);
	}
...
int http_fn_cfg(http_request_t* request) {
	http_setup(request, httpMimeTypeHTML);
	if (DRV_IsRunning("DeyeSolarman")) {
		poststr(request, "<!DOCTYPE html><html><head><title>AgriHT Config</title>");
		poststr(request, "<meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,user-scalable=no'>");
		poststr(request, "<style>body{margin:0;background:#eef5ff;color:#101828;font-family:Arial,Helvetica,sans-serif}.wrap{max-width:520px;margin:0 auto;padding:18px}h1{font-size:22px}.card{background:#fff;border:1px solid #bfdbfe;border-radius:8px;padding:14px;box-shadow:0 6px 20px #0f172a18}a,input{display:block;box-sizing:border-box;width:100%;margin:10px 0;padding:12px;border-radius:6px;border:1px solid #94a3b8;background:#fff;color:#111827;text-align:center;text-decoration:none;font-size:16px}.primary{background:#166534;color:white;border-color:#166534}.wm{text-align:center;color:#64748b;font-size:12px;margin-top:14px}</style>");
		poststr(request, "</head><body><div class='wrap'><h1>AgriHT Zero Export</h1><div class='card'>");
		poststr(request, "<a class='primary' href='/cfg_wifi'>Cau hinh WiFi</a>");
		poststr(request, "<a href='/ota'>OTA firmware</a>");
		poststr(request, "<a href='/index?restart=1' onclick=\"return confirm('Khoi dong lai thiet bi?')\">Khoi dong lai</a>");
		poststr(request, "<a href='/index'>Quay lai panel</a>");
		poststr(request, "</div><div class='wm'>Made by AgriHT &copy; 2026</div></div></body></html>");
		poststr(request, NULL);
		return 0;
	}
